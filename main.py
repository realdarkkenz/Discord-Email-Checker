...
{Fore.LIGHTBLUE_EX}Discord: {Fore.LIGHTCYAN_EX} https://discord.com/users/787475535483633684 """)
print("")
print(Fore.RED + f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'captcha-required' in json_response.get('message', '').lower() or 'captcha_sitekey' in json_response:
                ...
                return Fore.RED + f"{email} Not registered on Discord."
            return f"{email} Registration failed: {json_response}"
        elif response.status_code == 429:
            retry_after = json_response.get('retry_after', 10)
            print(f"{Fore.YELLOW}{email} Rate limit exceeded. Waiting {retry_after} seconds...")
            time.sleep(retry_after)
            return f"{email} Retried after rate limit."
        else:
            return f"{email} Unknown error. Status code: {response.status_code}, Response: {json_response}"
    except requests.exceptions.RequestException as e:
        return f"{email} Request error: {e}"
...
def check_emails_from_file(email_filename, live_filename, mfa_filename):
    ...
    for email in emails:
        ...
        if "registered discord" in lower_result:
            with open(live_filename, 'a') as live_file:
                live_file.write(email + "\n")
        if "2fa enabled" in lower_result:
...
# Kullanım
email_dosya_adi = "emails.txt"  
live_dosya_adi = "livehesap.txt"  
mfa_dosya_adi = "2fa_emails.txt"  
